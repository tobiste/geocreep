---
title: "Flow law sensitivity analysis"
...
edot_estimates0 <- sapply(models,
       function(m){
         creep_quartz(stress = stress0, temperature = temperature, fugacity = fugacity, pressure = pressure, grainsize = grainsize, model = m, propagate_err = FALSE)
       }) |> 
  setNames(models) |> 
  t() |> 
  t() |> 
...
edot_estimates1 <- lapply(models,
       function(m){
         creep_quartz(stress = median(stress1), temperature = temperature, fugacity = fugacity, pressure = pressure, grainsize = grainsize, model = m, sim = n)
       }) |> 
  setNames(models)
```
...
edot_quantiles1 |> 
  ...
  annotate('text', x =  10^(-14.5), y = length(models),  label = 'Natural geological strain rates', angle = 90, hjust = 1, vjust = .5, color = 'grey50', fontface = 3) +
  geom_pointrange(color = '#4E79A7') +
  geom_point(aes(x = mean), color = '#4E79A7', size = 1.5, shape = 0) +
...
fugacity2 <- ps_fugacity(pressure2, temperature2)
```
...
edot_estimates2 <- lapply(models,
       function(m){
         creep_quartz(stress = stress1, temperature = temperature2, fugacity = fugacity2, pressure = pressure2, grainsize = grainsize, model = m, sim = n)
       }) |> 
  setNames(models)
```
